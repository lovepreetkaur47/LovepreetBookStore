...
namespace LovepreetBook.DataAccess.Repository
{
    public class SP_Call : ISP_Call
    {
        ...
        private static string ConnectionString = ""; //needed to called procedure
        //Contructore to open Sql connection
        ...
        public Tuple<IEnumerable<T1>, IEnumerable<T2>> List<T1, T2>(string procedureName, DynamicParameters param =null)
        {
            using (SqlConnection sqlCon = new SqlConnection(ConnectionString))
            {
                ...
                var items2 = result.Read<T2>().ToList();

                if(item1 != null && items2 != null)
                {
